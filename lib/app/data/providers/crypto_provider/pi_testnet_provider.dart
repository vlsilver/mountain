...
class PiTestNetProvider extends GetConnect {
  ...
  Future<double> getBaseFees() async {
    ...
    if (result.statusCode == 200) {
      ...
      throw jsonDecode(result.body)['title'] ?? 'Calculator fee failure';
    }
  }
  ...
  Future<BigInt> getSequence({required String address}) async {
    ...
    if (result.statusCode == 200) {
      final sequenceString = jsonDecode(result.body)['sequence'];
      final sequence = BigInt.parse(sequenceString as String);
      return sequence;
    } else {
  ...
  Future<String> getInformationTransaction({required String hash}) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw jsonDecode(response.body)['title'] ??
          'Get tranactions information failure';
    }
  }
}
